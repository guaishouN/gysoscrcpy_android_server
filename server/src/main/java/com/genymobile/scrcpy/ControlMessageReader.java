...
public class ControlMessageReader {
    ...
    public ControlMessage next() {
        ...
        switch (type) {
            case ControlMessage.TYPE_INJECT_KEYCODE:
                ...
                break;
            case ControlMessage.TYPE_INJECT_CONTROL_EVENT:
                msg = parseInjectControl();
                break;
    ...
    private ControlMessage parseInjectControl() {
        if (buffer.remaining() < INJECT_KEYCODE_PAYLOAD_LENGTH) {
            return null;
        }
        int action = Binary.toUnsigned(buffer.get());
        ...
        int repeat = buffer.getInt();
        int metaState = buffer.getInt();
        return ControlMessage.createInjectControl(action, keycode, repeat, metaState);
    }
